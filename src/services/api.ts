...
export interface GamesProceedRequest {
  ...
  tags: string[];
  steamId?: string | null;
  /** Existing chat to continue. Omit to start a new chat (backend mints a new chatId). */
  chatId?: string;
  /**
   * Idempotency key (UUID v4). Generated by FE per user message.
   * Replayed requests with the same clientRequestId MUST NOT create a duplicate
   * USER message on backend (see PCAI-116).
   */
